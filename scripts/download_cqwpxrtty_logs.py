#!/usr/bin/env python3
"""
Download the full CQ WPX RTTY public log archive.

Directory layout:
    CQWPXRTTY/<year>/<callsign>.log

The script scrapes https://cqwpxrtty.com/publiclogs/ to discover year pages,
...
def discover_year_pages() -> Iterable[Tuple[str, str]]:
    ...
    for match in re.finditer(r"href=['\"](?P<year>(19|20)\d{2})/['\"]", html):
...
def download_log(year: str, log_url: str) -> dict[str, int]:
    """Download a single log file into CQWPXRTTY/<year>/."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Download CQ WPX RTTY public logs.")
    parser.add_argument(
        "--last",
        type=int,
        default=None,
    ...
    for year, page_url in pages:
        ...
        skip, list_hash, item_count = task_should_skip(
            TASK_LEDGER, task_key, [url for _y, url in logs]
        )
        if skip:
