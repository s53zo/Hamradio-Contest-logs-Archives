#!/usr/bin/env python3
"""
Download UA9QCQ Wednesday Mini-Test 80m UBN logs for selected dates.

Defaults to two oldest available dates to avoid unpublished results.
...
TEST_ID = "243"  # Wednesday Mini-Test 80m
REQUEST_TIMEOUT = 30
...
PROGRESS_LABEL = "Wednesday Mini-Test 80m"
DEFAULT_MODE = "CW"
DEFAULT_BAND = "80M"
...
def confirmed_qsos(
    ...
    for depth, cells in collector.rows:
        ...
        if int(time_val) < start_time:
            continue
        if not include_errors and cells[col["error"]].strip():
            continue
        freq = freq_val
...
def build_cabrillo(
    ...
    lines = [
        "START-OF-LOG: 3.0",
        "CREATED-BY: ua9qcq-wed-mini-test-80m-ubn-downloader",
        "CONTEST: WEDNESDAY-MINI-TEST",
        f"CALLSIGN: {call}",
...
def fetch_for_date(
    ...
    for entry in entries:
        ...
        stats.total_calls += 1
        dest_dir = output_dir / contest_date
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Fetch UA9QCQ UBN logs for Wednesday Mini-Test 80m."
    )
    parser.add_argument(
        "--year",
    ...
    parser.add_argument(
        "--out",
        type=Path,
        default=Path("WednesdayMiniTest80m"),
    ...
    parser.add_argument(
        ...
        type=int,
        default=1700,
        help="Earliest QSO time (HHMM) to include (default: 1700).",
    )
    parser.add_argument(
        "--include-errors",
        action="store_true",
        help="Include QSO rows with non-empty Error column.",
    )
    parser.add_argument(
        "--max-date-seconds",
        type=int,
    ...
    if not date_options:
        raise SystemExit("Unable to find date options for this contest/year.")
